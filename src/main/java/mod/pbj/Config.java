...
public class Config {
	...
	static {
		AUTO_RELOAD =
			BUILDER.comment("Enables auto-reloading of guns").defineEnum("autoReloadEnabled", AutoReload.CREATIVE);
		SCOPE_AIMING_MOUSE_SENSITIVITY = BUILDER.comment("Adjusts mouse sensitivity when using scopes")
											 .defineInRange("scopeAimingMouseSensitivity", 0.4D, 0.01D, 0.9D);
		RESET_AUTO_FIRE_PITCH_ENABLED = BUILDER.comment("Toggle to reset player pitch to original after auto fire")
		...
		EMERALD_EXCHANGE_RATE =
			BUILDER
				...
				.defineInRange("emeraldExchangeRate", 100.0D, 1.0D, 1000.0D);
		...
		PIP_SCOPE_COLOR_BALANCE_RED =
			BUILDER.comment("Sets pip scope red color balance. This options works for shaders only.")
				.defineInRange("pipScopeColorBalanceRed", 90, 0, 255);
		PIP_SCOPE_COLOR_BALANCE_GREEN =
			BUILDER.comment("Sets pip scope green color balance. This options works for shaders only.")
				.defineInRange("pipScopeColorBalanceGreen", 105, 0, 255);
		PIP_SCOPE_COLOR_BALANCE_BLUE =
			BUILDER.comment("Sets pip scope blue color balance. This options works for shaders only.")
				.defineInRange("pipScopeColorBalanceBlue", 110, 0, 255);
		CUSTOM_SHADERS_ENABLED = BUILDER.comment("Enables custom shaders").define("customShadersEnabled", true);
		EXPLOSION_DESTROY_BLOCKS_ENABLED =
			BUILDER.comment("Enables explosions to destroy blocks").define("explosionDestroyBlocksEnabled", true);
		ITEM_DROP_CHANCE = BUILDER.comment("Sets the probability of dropping some item(s) by monster mobs.")
							   .defineInRange("itemDropChance", 0.35D, 0.0D, 1.0D);
		MAX_ITEM_DROP_COUNT = BUILDER.comment("Sets the max count of items to be dropped by a monster mob.")
								  .defineInRange("maxItemDropCount", 7, 2, 64);
		BULLETS_BREAK_GLASS_ENABLED =
			BUILDER.comment("Enables bullets breaking glass").define("bulletsBreakGlassEnabled", true);
		HITSCAN_DAMAGE_MODIFIER = BUILDER.comment("Modifier to adjust damage dealt by hitscan weapons.")
									  .defineInRange("hitscanDamageModifier", 1.0D, 0.1D, 10.0D);
		HEADSHOT_DAMAGE_MODIFIER = BUILDER.comment("Modifier to adjust damage dealt by headshot.")
									   .defineInRange("headshotDamageModifier", 3.0D, 1.0D, 10.0D);
		IFRAMES = BUILDER.comment("Time in ticks before a player can be damaged again by a gun.")
					  .defineInRange("iframes", 1, 0, 20);
		...
		ADVANCE_IRIS_INTEGRATION_ENABLED = BUILDER.comment("Enables advanced integraiton with Iris shaders")
											   .define("advancedIrisIntegrationEnabled", true);
		FIRST_PERSON_ANIMATIONS_ENABLED =
			BUILDER.comment("Enables advanced first person animations").define("firstPersonAnimationsEnabled", true);
		THIRD_PERSON_ANIMATIONS_ENABLED =
			BUILDER.comment("Enables advanced third person animations").define("thirdPersonAnimationsEnabled", true);
